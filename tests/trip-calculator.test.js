...
test('Checking distance of trip from QEW to Highway 401', async () => {
    const costDistArr = await costOfTrip(interchanges, "QEW", "Highway 401");
    expect(costDistArr[0].toFixed(4)).toBe("35.0120")
})

test('Checking distance of trip from Highway 401 to QEW', async () => {
    const costDistArr = await costOfTrip(interchanges, "Highway 401", "QEW");
    expect(costDistArr[0].toFixed(4)).toBe("35.0120")
})

test('Checking distance of trip from QEW to QEW', async () => {
